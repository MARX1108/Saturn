...
const actorRepository_1 = require('../actorRepository');
...
describe('ActorRepository', () => {
  ...
  let actorRepository;
  beforeAll(async () => {
    const setup = await (0, testUtils_1.setupTestDb)();
    client = setup.client;
    db = setup.db;
    actorRepository = new actorRepository_1.ActorRepository(db);
  });
  afterAll(async () => {
    await (0, testUtils_1.teardownTestDb)(client, db);
  });
  beforeEach(async () => {
  ...
  describe('updateProfile', () => {
    it("should update an actor's profile", async () => {
      ...
      const updatedActor = await db
        .collection('actors')
        .findOne({ _id: new mongodb_1.ObjectId(actorId) });
    ...
    it("should return false if actor doesn't exist", async () => {
      ...
      const result = await actorRepository.updateProfile(
        new mongodb_1.ObjectId().toString(),
        { displayName: 'Updated Name' }
      );
      // Assert
      expect(result).toBe(false);
    ...
    it('should find followers of an actor', async () => {
      ...
      await db.collection('actors').insertMany([
        ...
        },
        {
          ...
          following: [new mongodb_1.ObjectId()], // Following someone else
        },
      ]);
      // Act
  ...
  describe('findFollowing', () => {
    it('should find users that an actor is following', async () => {
      // Arrange
      const actorId = new mongodb_1.ObjectId();
      const followedUser1 = new mongodb_1.ObjectId();
      const followedUser2 = new mongodb_1.ObjectId();
      await db.collection('actors').insertMany([
        {
          _id: actorId,
          preferredUsername: 'testuser',
        ...
        { _id: new mongodb_1.ObjectId(), preferredUsername: 'notfollowed' },
      ]);
      // Act
  ...
  describe('addFollowing and removeFollowing', () => {
    it('should add a user to following list', async () => {
      // Arrange
      const actorId = new mongodb_1.ObjectId();
      const targetId = new mongodb_1.ObjectId();
      await db.collection('actors').insertMany([
