...
import { createTestActor, createTestPost, createTestLike } from "../../../../tests/helpers/factories";
import { connectDB, disconnectDB, clearCollections } from "../../../../tests/helpers/dbHelper";
...
describe("PostService - Unlike Post", () => {
  ...
  describe("unlikePost", () => {
    it("should unlike a previously liked post", async () => {
      ...
      const result = await postService.unlikePost(post._id.toString(), actor._id.toString());

      // ASSERT - Verify the post was unliked
      expect(result).toBe(true);
      
      // Verify the database state
      const unlikedPost = await db.collection("posts").findOne({ _id: post._id });
    ...
    it("should return false for non-existent post", async () => {
      ...
      // ACT
      const result = await postService.unlikePost(nonExistentPostId, actor._id.toString());
    ...
    it("should not allow a different user to unlike another user's like", async () => {
      ...
      const postOwner = await createTestActor(db, { preferredUsername: "post-owner" });
      const liker = await createTestActor(db, { preferredUsername: "liker" });
      const otherUser = await createTestActor(db, { preferredUsername: "other-user" });
      ...
      const result = await postService.unlikePost(post._id.toString(), otherUser._id.toString());
      
      // ASSERT - Should fail (cannot unlike someone else's like)
      expect(result).toBe(false);
      
      // Verify the like still exists
      const postAfterAttempt = await db.collection("posts").findOne({ _id: post._id });
      expect(postAfterAttempt.likedBy).toEqual(expect.arrayContaining([liker._id]));
