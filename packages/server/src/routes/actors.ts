...
export default function configureActorRoutes(db: Db, domain: string) {
  ...
  // Create an instance of ActorService
  ...
  // Update actor
  router.put("/:username", upload.single("avatarFile"), async (req, res) => {
    try {
      const { username } = req.params;
  ...
  // Add separate route for ActivityPub actor endpoint
  router.get("/activitypub/:username", async (req, res) => {
    try {
      const { username } = req.params;
